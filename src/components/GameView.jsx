...
export default function GameView({
  ...
  return (
    <div className="w-full flex flex-col items-center space-y-4" data-purpose="view-gameview">
      ...
      <div className="flex items-center justify-center w-full max-w-md mx-auto my-1">
        <div 
          className="w-full bg-[#1b1007] p-1.5 rounded-xl border-2 border-amber-700/80 shadow-2xl flex items-center gap-2"
          role="group"
        ...
        >
          ...
          <button
            ...
            className={`flex-1 py-2 px-3 rounded-lg font-cinzel font-bold text-xs uppercase tracking-wider transition-all cursor-pointer flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-amber-400 ${
              viewMode === 'tasks'
                ? 'parchment-surface text-amber-950 border-2 border-amber-900 shadow-md font-black scale-[1.02]'
                : 'bg-wood-950/80 text-amber-300/70 hover:text-amber-200 hover:bg-wood-900 border border-amber-900/40'
            }`}
        ...
        <div className="lg:col-span-8 flex flex-col">
          ...
          {viewMode === 'habits' && (
            <div className="flex flex-col space-y-4" data-purpose="habits-mode-panel">
              ...
              <div className="guardian-card-frame p-4 sm:p-5 bg-wood-planks border-4 border-[#201308] space-y-3 shadow-2xl">
                
                {/* Ledger Description Plaque */}
                <div className="guardian-dark-inset rounded-lg py-2 px-3 flex items-center justify-between">
                  <div className="flex items-center gap-2 text-xs font-newsreader text-amber-300/90 italic">
                    <Coins className="w-4 h-4 text-amber-400 shrink-0" />
                    <span>Daily check-ins award escalating guild coins: +min(streak + 1, 10) GP. Single daily fortify lock.</span>
                  </div>
                  <div className="text-[11px] font-cinzel font-bold text-amber-300 uppercase px-2 py-0.5 rounded bg-black/60 border border-amber-600/40 shrink-0 ml-2">
                    Sole Coin Source
                  </div>
                </div>
                  ...
                  <div className="space-y-2.5" role="feed" aria-label="Habit Forge Ledger">
                    {habits.map((habit) => {
                      ...
                      return (
                        <div 
                          key={habit.id}
                          className={`rounded-xl border-2 transition-all flex items-center justify-between p-3 gap-3 ${
                            isCompleted 
                              ? 'bg-[#18110b]/90 border-stone-800 opacity-80' 
                              : 'bg-wood-900/90 border-amber-700/60 hover:border-amber-400 hover:bg-wood-850 shadow-md'
                          }`}
                        >
                          ...
                          >
                            ...
                            <div className="flex items-center space-x-3 min-w-0">
                              <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 border ${
                                isCompleted 
                                  ...
                                  : 'bg-amber-950/80 border-amber-600/50 text-amber-300 group-hover:scale-105'
                              }`}>
                              ...
                              <div className="min-w-0">
                                <div className="flex items-center space-x-2">
                                  ...
                                  <span className="text-[10px] font-cinzel uppercase px-1.5 py-0.2 rounded bg-stone-900 border border-stone-700 text-amber-400/80 shrink-0">
                                    {habit.category}
                                  </span>
                                </div>
                                <div className="text-[11px] font-newsreader text-stone-400 flex items-center gap-1 mt-0.5">
                                  <span>Streak:</span>
                                  <span className="font-bold text-amber-300">{currentStreakVal} Days</span>
                                  {habit.longest_streak > currentStreakVal && (
                                    <span className="text-stone-500 text-[10px]">(Record: {habit.longest_streak})</span>
                                  )}
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="shrink-0 flex items-center space-x-2">
                              {isCompleted ? (
                                ...
                                <div className="px-3 py-1.5 rounded-lg bg-stone-950 border border-stone-700 flex items-center gap-1.5 text-stone-400 text-xs font-cinzel">
                                  <Check className="w-3.5 h-3.5 text-emerald-400" />
                                  <span>Sealed Today</span>
                                </div>
                              ) : (
                                /* Active Fortify Seal */
                                <div className="wax-seal px-3.5 py-1.5 rounded-lg border border-amber-400 shadow flex items-center gap-1.5 text-amber-200 text-xs font-cinzel font-bold group-hover:scale-105 transition-transform">
                          ...
                          {onDeleteHabit && (
                            <button
                              ...
                              className="p-1.5 rounded-lg text-stone-500 hover:text-red-400 hover:bg-red-950/40 transition-colors focus:outline-none focus:ring-1 focus:ring-red-400 cursor-pointer shrink-0"
